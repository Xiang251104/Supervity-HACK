...
`app.routers.ap_runs.start_run` derives the persisted default trigger source only after Auto's canonical invoice is available. It treats normalized `EMAIL` as Outlook-originated, but preserves an explicitly non-default source. After policy gating, the router commits the completed run, decision, and Workbench item before the best-effort Slack notification. It then appends and commits a `RunEvent(event_type="integration_activity")` using a run-local next-sequence allocation and standardized Slack outcome payload, so integration health is based on real run activity rather than a synthetic probe. This allocation preserves normal event ordering; no migration is in scope to add a database uniqueness guarantee.
